...
% no decaying 
 	...
 	[ARF,RCF,PE2] = mvarOaa(Y, Pmax, Mode0);	
	...
	Y2 = flipud(Y); 
 	[ARB,RCF,PE] = mvarOaa(Y2, Pmax, Mode0);	
	Y2(1,isnan(Y2(1,:))) = 0; 
	z  = [];
	for k = 2:size(Y2,1)
	...
	Y(isnan(Y)) = Z(isnan(Y));
 	...
 	[ARF,RCF,PE] = mvarOaa(Y, Pmax, rem(Mode,100));	
...
% no decaying 
	...
	Y  = flipud(Y);
 	[ARB,RCF,PE] = mvarOaa(Y, Pmax, Mode0);	
	...
	Y1 = flipud(Y1);
 	[ARF,RCF,PE] = mvarOaa(Y1, Pmax, rem(Mode,100));	


elseif Mode>=2200,  % forward predictions, 
% assuming that past missing values are already IMPUTED with the prediction value + innovation process
% no decaying 
 	[ARF,RCF,PE] = mvarOaa(Y, Pmax, Mode0);	
 	c = chol(PE(:,M*Pmax+(1:M))); 
	Y1 = Y; 
	Y1(1,isnan(Y1(1,:))) = 0; 
	z  = [];
...
elseif Mode>=1300,  % backward prediction
	...
	Y2 = flipud(Y2);
 	[ARF,RCF,PE] = mvarOaa(Y2, Pmax, rem(Mode,100));	
...
% assuming that 'past' missing values are ZERO
	...
	Y2 = flipud(Y2);
 	...
 	[ARF,RCF,PE] = mvarOaa((Y1+Y2)/2, Pmax, rem(Mode,100));	
...
ARF = reshape(ARF,M,M,Pmax);
PE = PE(:,Pmax*M+1:end);
% ResVar = PE(:,p*M+[1:M]); %residual variance for model order p and M is #chan.
